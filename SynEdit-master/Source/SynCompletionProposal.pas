...
begin
  ...
  begin
    if FTitle <> '' then
      FHeightBuffer := FTitleFontHeight + 4
    else
      FHeightBuffer := 0;
...
procedure TSynBaseCompletionProposal.ExecuteEx(s: UnicodeString; x, y: Integer; Kind : SynCompletionType);
  ...
  begin
    ...
    begin
      tmpX := GetWorkAreaWidth - tmpWidth - 5;
      if tmpX < 0 then
        tmpX := 0;
    end;
...
procedure TSynCompletionProposal.HandleOnHide(Sender: TObject);
begin
