...
begin
  ...
  begin
    ...
    if (ErrorCode <> 0) and (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, nil,
...
begin
  ...
  if Value >= ActiveColorMap.FCount then
    Error(sBadColorIndex);
  BYTE(PAnsiChar(longInt(Scanline[y]) + x)^) := Value;
end;
// 2006.10.09 <-
...
{$IFDEF REGISTER_TGIFIMAGE}
  ...
  CF_GIF := RegisterClipboardFormatA(PAnsiChar(sGIFImageFile));
