...
uses
  Types, Classes, Graphics, Controls, System.UITypes, Math, Dialogs, SysUtils, ExtCtrls,
...
type
  ...
  protected
    ...
    procedure DrawButton(const ARect: TRect; Checked, Hover, Down, Indeterminate: Boolean;
  ...
  public
    ...
    procedure PaintValue; override;
    procedure Toggle(const ButtonIndex: Integer);
  end;
  ...
  TNxComboBoxDisplay = class(TNxTextItemDisplay)
  protected
    procedure PaintPreview(PreviewRect: TRect); override;
  end;

  TNxDateItemDisplay = class(TNxTextItemDisplay)
...
FUNCTION DefineBrushPattern(CONST  BrushPattern:  TBrushPattern): Graphics.TBitmap;
  VAR
    i     :  INTEGER;
    j     :  INTEGER;
    Row   :  pByteArray;
...
procedure DrawMatrix(Canvas: TCanvas; Rect: TRect);
var
  PatternBitmap: Graphics.TBitmap;
begin
  try
    PatternBitmap := DefineBrushPattern(Pattern);
    Canvas.Brush.Bitmap := PatternBitmap;
...
procedure TNxToolbarItemDisplay.DrawButton(const ARect: TRect; Checked, Hover, Down, Indeterminate: Boolean;
  ButtonIndex: Integer);
var
  X, Y: Integer;
  ...
  var
    ...
    if Hover then
    begin
      Canvas.Brush.Color := IfThen(Indeterminate, clSilver, clBtnFace);
    ...
    begin
      if Checked or Down then
      begin
        Canvas.Brush.Color := IfThen(Indeterminate, clSilver, cl3DLight);
        Canvas.FillRect(ARect);
        Frame3D(Canvas, FrRect, clBtnShadow, clBtnHighlight, 1);
      end else if Indeterminate then
      begin
...
begin
  if Checked or Down or Hover or Indeterminate then
    case (Item.GetParentControl as TNextInspector).Style of
      ...
      psOffice2007, psOffice2010: DrawOffice2007Button;
...
begin
  ...
  begin
    ...
    begin
      ...
      end else DrawButton(GetButtonRect(i), Buttons[i].Checked, i = FHoverIndex, i = FDownIndex, Buttons[i].Indeterminate, i);
    end;
...
begin
  ...
  begin
    case Style of
      ...
      psOffice2007, psOffice2010:
      begin
        Canvas.Brush.Color := SchemeColor(seBtnFace);
        Canvas.FillRect(ClientRect);
      end;
      psWhidbey:
      begin
...
begin
  ...
  begin
    ...
    if InRange(ButtonIndex, 0, Pred(Count)) then
    begin

      if Buttons[ButtonIndex].AutoCheck
        then Toggle(ButtonIndex) { Unchecked/Grayed/Checked }
      else
      begin
        { Basic Click }
...
begin
  KeyIndex := NumberKey(Key);
  KeyIndex := FindButton(KeyIndex);
  if KeyIndex = -1 then Exit;
  with Item as TNxToolbarItem do
  begin
    if Buttons[KeyIndex].AutoCheck then Toggle(KeyIndex);
  end;
end;

procedure TNxToolbarItemDisplay.Toggle(const ButtonIndex: Integer);
var
  ClickButton: TNxToolbarItemButton;
begin
  with Item as TNxToolbarItem do
  begin
    ClickButton := Buttons[ButtonIndex];

    if AllowIndeterminate then
    begin

      if ClickButton.Indeterminate then
      begin
        ClickButton.Indeterminate := False;
        ClickButton.Checked := True;
      end else
      begin
        { note: Button may be Indeterminate,
                only if is not checked (!) }
        if ClickButton.Checked then
        begin
          { note: Need to be called to trigger
                  Change method }
          ClickButton.Indeterminate := False;
          ClickButton.Checked := False;
        end else
        begin
          ClickButton.Indeterminate := True;
          ClickButton.Checked := False;            
        end;
    ...
    else ClickButton.Checked := not ClickButton.Checked; { Normal }
...
procedure TNxCheckBoxItemDisplay.DrawButton(const ARect: TRect; Checked,
  Hover, Down, Indeterminate: Boolean; ButtonIndex: Integer);
  ...
  begin
    ...
    begin
      Index := 5;
      if Hover then Index := 6;
      if Down then Index := 7;
      if not Item.Enabled then Index := 8;
    end
    else if Indeterminate then
    begin
      Index := 9;
      if Hover then Index := 10;
      if Down then Index := 11;
      if not Item.Enabled then Index := 12;
    end
    else
    begin
      Index := 1;
      if Hover then Index := 2;
      if Down then Index := 3;
      if not Item.Enabled then Index := 4;
    end;
  ...
  begin
    ...
    if Indeterminate then Flags := Flags or DFCS_INACTIVE or DFCS_CHECKED;
  ...
  begin
    ...
    case ColorScheme of
      csSilver, csSilver2010: ResName := ResName + '1';
      csBlue, csBlue2010: ResName := ResName + '0';
      csBlack, csBlack2010: ResName := ResName + '2';
    end;
  ...
  begin
    ...
    ABitmap := Graphics.TBitmap.Create;
    ResName := 'CHECKBOX2010_' + IntToStr(Index);
    case ColorScheme of
      csSilver, csSilver2010: ResName := ResName + '1';
      csBlue, csBlue2010: ResName := ResName + '0';
      csBlack, csBlack2010: ResName := ResName + '2';
  ...
  case TNxCheckBoxItem(Item).Style of
    ...
    begin
      case Style of
        ...
        psOffice2010: DrawOffice2010Mark;
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ImgTop := PreviewRect.Top + (PreviewRect.Bottom - PreviewRect.Top) div 2 - Images.Height div 2;
      ImgLeft := PreviewRect.Left + 1;
      Images.Draw(Canvas, ImgLeft, ImgTop, ItemIndex);
    end;
  end;
end;
