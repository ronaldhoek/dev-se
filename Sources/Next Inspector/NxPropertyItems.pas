...
// Delete next line to disable creating Inplace Editor when item is created
{$UNDEF NEW_EDITORCREATION}
...
uses
  Classes, Types, UITypes, Controls, Graphics, ComCtrls, ImgList, Dialogs, TypInfo,
...
type
  ...
  TPropertiesStyle = (psDefault, psOffice2003, psOffice2007, psOffice2010, psWhidbey);
  TPropertyItemPart = (ipAll, ipButton, ipCaption, ipCategory, ipMargin, ipPlus,
  ...
  TChangeKind = (ckAdded, ckButton, ckCaption, ckDeleted, ckCollapse, ckDisable, ckExpand,
    ckUndefined, ckValueRepaint, ckValueChanged, ckPreCollapse, ckSilentValue,
    ckSuffixChanged, ckValueMember, ckToolButtonClick, ckToolButtonRedraw,
    ckRefresh, ckVisible, ckMinimize, ckMaximize, ckIndeterminateChanged);
  ...
  TItemDataType = (idtBoolean, idtDateTime, idtFloat, idtInteger, idtString, idtLookup, itdMemo);
  ...
  private
    ...
    FOnDrawPreview: TCustomPreviewDrawEvent;
    ...
    FSuffixDisplayMode: TItemsDisplayMode;
    FSuffixes: TStrings;
    ...
    function GetChildHeight: Integer;
  ...
  protected
    ...
    procedure Change(Kind: TChangeKind; Parameter: Integer = -1); virtual;
    procedure Changed;
  ...
  public
    ...
    function GetStoreValue: WideString; virtual;
    ...
    procedure UpdateField(S: WideString); virtual;
    ...
    property ChildHeight: Integer read GetChildHeight;
    ...
    property Shown: Boolean read GetShown;
  ...
  published
    ...
    property ShowMinimize: Boolean read FShowMinimize write SetShowMinimize default False;
    ...
    property SuffixDisplayMode: TItemsDisplayMode read FSuffixDisplayMode write FSuffixDisplayMode default dmDefault;
    ...
    property OnDrawPreview: TCustomPreviewDrawEvent read FOnDrawPreview write FOnDrawPreview;
  end;
  ...
  private
    ...
    function GetItem(Index: Integer): TNxPropertyItem;
    function GetItemByCaption(Caption: string): TNxPropertyItem;
  ...
  protected
    ...
    function GetExpandedChildCount(List: TList; Item: TNxPropertyItem): Integer;
    function GetItemClientHeight(Item: TNxPropertyItem): Integer;
    function GetVisibleChildCount(List: TList; Item: TNxPropertyItem): Integer;
  ...
  public
    ...
    property ItemByCaption[ItemCaption: string]: TNxPropertyItem read GetItemByCaption;
    property ItemByName[ItemName: string]: TNxPropertyItem read GetItemByName;
  ...
  public
    ...
    procedure PaintMinimizeButton; virtual;
...
function TNxPropertyItem.GetChildHeight: Integer;
begin
  Result := FItems.GetItemClientHeight(Self);
end;
...
begin
  ...
  begin
    ...
    if FItems.Node[i].FItemHeight = 0
      then Step := rh
      else Step := FItems.Node[i].ClientHeight;//FItemHeight;
    Inc(y, Step + grid);
  end;
end;
...
begin
  ...
  begin
    Result := Result and (ParentItem.Shown and not ParentItem.Hidden) and ParentItem.Expanded;
  end else
  begin
...
begin
  ...
  FExpanded := Value;
  Delta := 0;
  if not FExpanded then Delta := Items.GetItemClientHeight(Self);

  FItems.SetExpanded(Self, FExpanded);

  if FExpanded then Delta := Items.GetItemClientHeight(Self);

  if Count > 0 then
  begin
    if FExpanded then Change(ckExpand, Delta) else Change(ckCollapse, Delta);
  end;
end;
  ...
  if Assigned(AParentControl) then
        ...
        begin
          Include(FItemState, VCLRead);
            ...
            tkString, tkLString{$IFDEF DELPHI2009UP}, tkUString{$ENDIF}: AsString := GetStrProp(AObject, AProperty);
            tkInteger:
            begin
  ...
  if Assigned(AParentControl) then
        ...
        ResolveProperty(AObject, AProperty);
      	PropInfo := GetPropInfo(AObject, AProperty);
       	if Assigned(PropInfo) then
					case PropInfo^.PropType^.Kind of
            tkString, tkLString{$IFDEF DELPHI2009UP},tkUString{$ENDIF}: SetStrProp(AObject, AProperty, AsString);
            tkInteger:
            begin
...
procedure TNxPropertyItem.SetPreviewBorder(const Value: Boolean);
begin
...
begin
  Result := nil;
  if FCount > 0 then Result := FItems[AbsoluteIndex + 1];
end;
...
begin
  ...
  FEditor.PreviewBorder := PreviewBorder;
  FEditor.Color := Color;
  FEditor.Font.Assign(ValueFont);
  { AutoSelect }
  if FAutoSelect then FEditor.SelectAll else
  begin
    FEditor.SelStart := Pred(Length(FEditor.Text));
  end;
end;
...
begin
  ...
  FSuffixCombo.DisplayMode := SuffixDisplayMode;
end;
...
procedure TNxPropertyItem.SetDrawingOptions(const Value: TDrawingOptions);
begin
  FDrawingOptions := Value;
  Changed;
end;

function TNxPropertyItem.GetStoreValue: WideString;
begin
  Result := AsString;
end;

procedure TNxPropertyItem.UpdateField(S: WideString);
begin

end;

procedure TNxPropertyItem.DoDrawPreview(PreviewRect: TRect);
begin
  if Assigned(FOnDrawPreview) then FOnDrawPreview(Self, PreviewRect);
end;

procedure TNxPropertyItem.SetMinimized(const Value: Boolean);
begin
  FMinimized := Value;
  if FMinimized then Change(ckMinimize) else Change(ckMaximize);
end;

function TNxPropertyItem.GetClientHeight: Integer;
begin
  case FMinimized of
    True: Result := TNextInspector(FItems.Owner).RowHeight;
    else Result := ItemHeight;
  end;
end;

procedure TNxPropertyItem.SetShowMinimize(const Value: Boolean);
begin
  FShowMinimize := Value;
  Change(ckCaption);
end;

function TNxPropertyItem.GetExpandedCount: Integer;
begin
  Result := FItems.GetExpandedChildCount(FItems.FItemsList, Self);
end;
...
begin
  Result := nil;
  if Index < Flist.Count then Result := TNxPropertyItem(FList[Index])
end;
...
function TNxPropertyItems.GetExpandedChildCount(List: TList;
  Item: TNxPropertyItem): Integer;
var
  i: Integer;
  AItem: TNxPropertyItem;
begin
  ...
  while (i < List.Count)
    and (TNxPropertyItem(List[i]).FLevel > Item.FLevel) do
  begin
    AItem := TNxPropertyItem(List[i]);
    if (AItem.Visible) then
    begin
      if AItem.HasChildren then
      begin
        if not AItem.Expanded then Inc(i, AItem.ChildCount);
      end;
      Inc(Result);
    end;
    Inc(i);
...
//boki: moved up, before item is deleted from list
  ...
  DoChange;
  //^call Invalidate and UpdateScrollbar
end;
...
procedure TNxPropertyItems.AddItem(Parent, Item: TNxPropertyItem;
  Position: TAddPosition; UpdateFont: Boolean);
var
  Pos: Integer;
begin
  Item.FItems := Self;
...
function TNxPropertyItems.GetVisibleChildCount(List: TList; Item: TNxPropertyItem): Integer;
var
  i: Integer;
  AItem: TNxPropertyItem;
begin
...
function TNxPropertyItems.GetItemClientHeight(
  Item: TNxPropertyItem): Integer;
var
  i: Integer;
  AItem: TNxPropertyItem;
begin
  ...
  begin
    AItem := GetItem(i);
    if AItem.Level <= Item.Level then Exit;
    if AItem.HasChildren and not AItem.Expanded then
    begin
      Inc(i, AItem.ChildCount);
    end;
    if AItem.Visible then
    begin
      Inc(Result, TNxCustomInspector(FOwner).GetItemHeight(AItem));
      Inc(Result, TNxCustomInspector(FOwner).GetGridSpace);
    end;
    Inc(i);
  end;
end;

function TNxPropertyItems.GetItemByCaption(
  Caption: string): TNxPropertyItem;
var
  i: Integer;
begin
  Result := nil;
  for i := 0 to Count - 1 do
    if CompareText(Item[i].Caption, Caption) = 0 then
    begin
      Result := Item[i];
      Exit;
    end;
...
begin
  if (Item.Level > 0) or (Item.Value <> '') then
    with Canvas do
    begin
...
begin
  ...
  begin
    ...
    case Style of
      ...
      begin
        if IsTop then
        begin
          FromColor := SchemeColor(seGridHeaderGradientStart, ColorScheme);
          ToColor := SchemeColor(seGridHeaderGradientEnd, ColorScheme);
          DrawVertGradient(Canvas, ClientRect, FromColor, ToColor);
          if Selected then
          begin
            FocusRect := Rect(Item.Indent + spaMargin, ClientRect.Top, Item.Indent + spaMargin + TextWidth(Item.Caption) + 4, ClientRect.Bottom);
            if (Item.ImageIndex > -1) and (Assigned(Images)) then FocusRect.Right := FocusRect.Right + Images.Width + spaImageToText;
        ...
        begin
          ...
          begin
            Brush.Color := SchemeColor(seHighlight, ColorScheme);
            FillRect(ClientRect);
          end;
        end;
      end;
  ...
  {$IFDEF NX_DEBUG}
  TGraphicsProvider.DrawTextRect(Canvas, CaptionRect, taLeftJustify, BoolToStr(Item.Hidden, True) + ' ' + Item.Caption);
  {$ELSE}
  TGraphicsProvider.DrawTextRect(Canvas, CaptionRect, taLeftJustify, Item.Caption);
  {$ENDIF}

  { Draw MinimizeButton }
  if Item.ShowMinimize then PaintMinimizeButton;
end;

procedure TNxItemDisplay.PaintPreview(PreviewRect: TRect);
begin
  Item.DoDrawPreview(PreviewRect);
end;
    ...
    if Assigned(Inspector.Glyphs)  and (Inspector.Glyphs.Count>0) then
      ...
      begin
        ...
        case Style of
          ...
          psOffice2003, psWhidbey, psOffice2007, psOffice2010:
          begin
...
procedure TNxItemDisplay.PaintMinimizeButton;
var
  ThemeItem, X, Y, CaptionLeft, CaptionRight, CaptionBottom: Integer;
  CaptionRect, BtnRect: TRect;
  Btn: Graphics.TBitmap;
  Inspector: TNxCustomInspector;
begin
  CaptionLeft := Item.Indent + spaMargin + spaTextStart;
  CaptionRect := Rect(CaptionLeft, ClientRect.Top, ClientRect.Right, ClientRect.Bottom);
  CaptionRight := CaptionRect.Right - 12;
  CaptionBottom := ClientRect.Bottom - 10;
  with ClientRect do
  begin
    Inspector := TNxCustomInspector(Item.FItems.Owner);
    if Assigned(Inspector.Glyphs) and (Inspector.Glyphs.Count>0) then
      ...
      begin
        ...
        case Style of
          psDefault:
          begin
            if not Item.Minimized then ThemeItem := tiExpanded else ThemeItem := tiCollapsed;
            if daVisualStyles in Attributes
              then ThemeRect(Item.GetParentControl.Handle, Canvas.Handle, BtnRect, teTreeView, tcExpandingButton, ThemeItem)
              else DrawExpandingButton(Canvas, Point(X, Y), not Item.Minimized, Item.Level = 0, FMarginColor);
          end;
          psOffice2003, psWhidbey, psOffice2007, psOffice2010:
          begin
            Btn := Graphics.TBitmap.Create;
            if not Item.Minimized then Btn.LoadFromResourceName(HInstance, 'OFFICE2003COLLAPSE')
              else Btn.LoadFromResourceName(HInstance, 'OFFICE2003EXPAND');
            TDrawProvider.ApplyBitmap(Canvas, BtnRect.Left, BtnRect.Top, Btn);
            Btn.Free;
          end;
