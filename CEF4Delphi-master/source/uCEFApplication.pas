...
type
  TCefApplication = class
    protected
      ...
      FCookieManager                 : ICefCookieManager;
    ...
    public
      ...
      property CookieManager: ICefCookieManager read FCookieManager write FCookieManager;
...
begin
  ...
  if (length(FCookies) > 0) then
    begin
      TempThread := TCEFCookieInitializerThread.Create(FCookies, FPersistSessionCookies, Self);
      {$IFDEF DELPHI14_UP}
      TempThread.Start;
      {$ELSE}
      TempThread.Resume;
...
procedure TCefApplication.ClearCache;
var TempEvent: ICefWaitableEvent;
TempCallBack: TCefEventCompletionCallback;
begin
  try
  if (length(FCookies) > 0) then
      begin
      ...
      FCookieManager.FlushStore(TempCallBack);
      end;
  finally
      TempEvent := nil;
      TempCallBack := nil;
    ...
    try
      if (length(FCookies) > 0) then
        begin
          ...
          FApp.CookieManager := TempCookieManager;
