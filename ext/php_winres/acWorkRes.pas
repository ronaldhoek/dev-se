...
//uses Windows { Alx madStrings, madTools, SysUtils};
...
function BeginUpdateResourceS(const Filename: String; const delExistingRes: Boolean): DWORD; stdcall;
begin
  Result:= BeginUpdateResourceW(PWideChar(WideString(Filename)), delExistingRes);
end;
...
function EndUpdateResource(const hUpdate: DWORD; const Discard: Boolean): Boolean; stdcall;
begin
  Result:= EndUpdateResourceW(hUpdate, Discard);
end;
  ...
  begin
    ...
    if dword(name) and $FFFF0000 = 0 then begin
      ...
      while (ppr1^ <> nil) and (ppr1^^.name <> '') and
            (CompareString(LANG_ENGLISH, NORM_IGNORECASE,
                           pchar(string(ppr1^^.name)), Length(ppr1^^.name),
                           pchar(       s1          ), Length(s1         )) = 1) do
        ppr1 := @ppr1^.next;
      result^.name := name;
      inc(tree^.namedItems);
    end;
...
function ReplaceIconGroupResourceS(const hUpdate: DWORD; const Name: PChar; const Language: Word; const IconFile: String): Boolean; stdcall;
begin
  Result:= LoadIconGroupResourceW(hUpdate, PWideChar(Name), Language, PWideChar(WideString(IconFile)));
end;
